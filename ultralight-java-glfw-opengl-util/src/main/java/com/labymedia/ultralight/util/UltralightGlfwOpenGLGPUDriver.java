...
public class UltralightGlfwOpenGLGPUDriver implements UltralightOpenGLGPUDriver {
   ...
   */
  @Override
  public void bindTexture(int textureId, UltralightGlfwOpenGLWindow window) {
    this.driverNative.bindTexture(textureId, window.postAndWait(() -> window.getView().renderTarget().getTextureId()));
  }

  @Override
  public int getTextureId(UltralightGlfwOpenGLWindow window) {
    return driverNative.getGlTextureId(window.postAndWait(() -> window.getView().renderTarget().getTextureId()));
  }
